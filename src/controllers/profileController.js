...
import { getProfile, updateProfile, getUserPosts, getPublicProfile, getPublicUserPosts, searchProfiles } from "../services/profileService.js";
...
export const searchUsers = asyncHandler(async (req, res) => {
  const q = req.query.q ? String(req.query.q).trim().slice(0, 80) : "";
  if (q.length < 2) return res.json([]);
  res.json(await searchProfiles(q));
});
